...
def sentiment_counts(data, target, figsize):
    ...
    ax =sns.countplot(x=target, data=data, dodge=False, hue=target,  order= data[target].value_counts().index, hue_order =data[target].value_counts().index,  palette=['#dcf4cb', '#afe586'], legend='full');
    ax.set(xticklabels=[]) 
    ax.set(ylabel=None) 
